...
val = 20
coordinates = Queue(maxsize=0) # An array that stores the coordinates of the X,Y of all the Sensors.
coordinates.put(0)
...
class ServerToTalkToReact(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        global coordinates
        self._set_headers()
        self.wfile.write(json.dumps(coordinates.get())) #Sends the first co-ordinates from co-ordinate list
...
class TalktoArduino(threading.Thread):
    def __init__(self, name):
        ...
        self.name = name
        self.client = M2XClient('7efd78739762999168565cacb8be97e6')
        self.device = client.device('94b177d7f116f44cf373b9dae7e0b0de')
        self.stream = device.stream('coordinates')
